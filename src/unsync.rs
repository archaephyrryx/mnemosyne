...
impl<T: Clone, E> Mnemosyne<T, E> {
    ...
    /// It is possible to inspect the last value that was successfully polled using the [`peek`] method, which may be useful
    /// if the computation is one that should not be attempted immediately after it fails.
    ///
    /// # Returns
    ///
